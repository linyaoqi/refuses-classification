...
class NodeLookup(object):  ##将整数节点ID转化为可读标签
...
def run_inference_on_image(image):  ##推理图片
  if not tf.gfile.Exists(image):
    tf.logging.fatal('File does not exist %s', image)
  image_data = tf.gfile.FastGFile(image, 'rb').read()

  create_graph(FLAGS.model_dir)  ##创建图形
  with tf.Session() as sess:  ##创建本地session，是客户端与整个tensorflow系统交互的接口
    # Some useful tensors:
    ...
    node_lookup = NodeLookup(uid_chinese_lookup_path='./data/imagenet_2012_challenge_label_chinese_map.pbtxt', \
                                model_dir=FLAGS.model_dir)  ##将节点ID转化为可读标签

    top_k = predictions.argsort()[-FLAGS.num_top_predictions:][::-1]
    
    for node_id in top_k:  ##打印推理结果
      human_string = node_lookup.id_to_string(node_id)
      score = predictions[node_id]
      print('%s (score = %.5f)' % (human_string, score))
      #print('node_id: %s' %(node_id))
"""下载和提取模型文件 `inception-2015-12-05.tgz` """
def maybe_download_and_extract():
  ...
  if not os.path.exists(filepath):
    def _progress(count, block_size, total_size):  ##进程
...
def main(_):
  ...
  image = (FLAGS.image_file if FLAGS.image_file else
           os.path.join(FLAGS.model_dir, 'cropped_panda.jpg'))  ##读取图片
